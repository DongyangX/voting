import { getProgram, truncate, voteCandidate } from '@/services/blockchain'
...
const Contestant: React.FC<{ contestant: ContestantStruct; poll: PollStruct }> = ({
  ...
  const voteContestant = async () => {
    ...
    let program = getProgram(connection, wallet)

    await toast.promise(
      new Promise<void>((resolve, reject) => {
        voteCandidate(program, poll.id, contestant.id)
          .then((tx: any) => {
            console.log(tx)
            resolve(tx)
          })
  ...
  return (
    <div className="flex justify-start items-center space-x-2 md:space-x-8 mt-5 md:mx-auto">
      ...
      >
        ...
        >
          {walletAddress && contestant.voters.includes(walletAddress) ? 'Voted' : 'Vote'}
        </button>
